            ...
            with self.view_client.get(
                ...
                if response.status_code == 200:
                    ...
                    if 'application/json' in content_type:
                        ...
                        response.success()  # Accept HTML as success for elasticity test
                else:
                    response.failure(f"Status code: {response.status_code}")
        except Exception as e:
...
class BurstShape(LoadTestShape):
    ...
    def tick(self):
        ...
        burst_intervals = [
            (120, 180),  # Burst 1: 2–3 min
            (240, 300),  # Burst 2: 4–5 min
            (360, 420),  # Burst 3: 6–7 min
        ]

        in_burst = any(start <= run_time < end for start, end in burst_intervals)
        user_count = 1000 if in_burst else 333
        spawn_rate = 100  # Fast enough to reach target user count within seconds
